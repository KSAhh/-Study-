...
```
  - `$ git init` : git 배포  
  - `$ git status` : 상태확인. 어떠한 파일을 수정했는지 확인 (빨간색 파일)  
  - `$ git add .` : git에 .을 추가한다. / "."은 모든 것을 의미  
  - `$ git status` : (초록색 파일)  
  - `$ git commit -m "설명"` : 설명 작성  
  - `$ git config --global user.email "ID@EAMIL" : commit이 안되는 경우, github이메일 정보 입력 / 이후에 다시 commit하기!  
  - `$ git remote add origin 깃허브 레포지토리 링크` : github 저장소와 연동  
  - `$ git push -u origin master` : master branch에 업로드  
```
